  ...
  return (
    <div className="Menu">
      <div className={`menu-left ${menu ? 'on' : ''}`}>
        ...
        <Tab name="브랜드 관리" hover='거래처 관리' path={`/brands?page=1&pagesize=${pageSize}`} search={brandsPage.searchUrl} img={brand_icon} />
        <Tab name="상품관리" hover='상품 관리' path={`/products?page=1&pagesize=${pageSize}`} search={productsPage.searchUrl} img={good_icon} />
        <Tab name={location.pathname === '/' ? "발주/반품 내역" : "발주/반품 내역 조회"}  
          hover='발주/반품 내역' path='/orders' search={ordersPage.searchUrl} img={order_icon} />
